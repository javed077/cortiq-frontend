...
const InputField = ({ label, value, onChange }: any) => (
  <div style={{ position: "relative" }}>
    ...
    <input type="text" value={value} onChange={e => onChange(e.target.value)}
      style={{ width: "100%", background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 8, padding: "11px 14px", color: "white", fontSize: 13, fontFamily: "'Space Mono', monospace", outline: "none", boxSizing: "border-box", transition: "border-color 0.2s" }}
...
export default function Home() {
  ...
  const API = process.env.NEXT_PUBLIC_API_URL ?? "https://cortiq-backend.onrender.com";
  ...
  // ── saveToHistory — inside the component ──────────────────────────────────
  function saveToHistory(data: any, formData: any) {
    ...
    if (daysDiff > 0) {
      ...
      localStorage.setItem("cortiq_streak", JSON.stringify({
        current, longest, lastCheckin: todayStr,
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      // ── SAVE TO HISTORY RIGHT AFTER setResult ──
      ...
      // save full result for /improve page
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .analyze-btn { width: 100%; padding: 16px; background: ${ACCENT}; color: #050505; font-family: 'Space Mono', monospace; font-weight: 700; font-size: 13px; letter-spacing: 0.15em; text-transform: uppercase; border: none; border-radius: 10px; cursor: pointer; transition: all 0.2s; animation: pulse-ring 2s infinite; }
        ...
        .improve-btn { width: 100%; padding: 16px; background: transparent; color: ${ACCENT}; font-family: 'Space Mono', monospace; font-weight: 700; font-size: 13px; letter-spacing: 0.15em; text-transform: uppercase; border: 1px solid ${ACCENT}; border-radius: 10px; cursor: pointer; transition: all 0.2s; }
        ...
        <div style={{ maxWidth: 900, margin: "0 auto", position: "relative", zIndex: 1 }}>
          ...
          <div className="card fade-up" style={{ animationDelay: "0.1s", marginBottom: 20 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 24 }}>
              ...
              <span style={{ fontFamily: "'Space Mono', monospace", fontSize: 10, color: "rgba(255,255,255,0.25)" }}>· fields marked * are required</span>
            </div>
            ...
            <div style={{ marginTop: 48, display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              <div className="card fade-up">
                ...
                <div style={{ width: "100%", height: 8, background: "rgba(255,255,255,0.06)", borderRadius: 4, overflow: "hidden" }}>
                  <div style={{ height: "100%", borderRadius: 4, width: `${result.risk_score}%`, background: result.risk_score > 60 ? "linear-gradient(90deg,#FFB800,#FF4444)" : `linear-gradient(90deg,${ACCENT},#00e5ff)`, boxShadow: `0 0 12px ${result.risk_score > 60 ? "#FF4444" : ACCENT}80`, transition: "width 1s cubic-bezier(.4,0,.2,1)" }} />
                </div>
                  ...
                  <div className="card fade-up">
                    ...
                    {strategy.map((s, i) => (
                      <div key={i} style={{ display: "flex", gap: 12, marginBottom: 14, paddingBottom: 14, borderBottom: i < strategy.length - 1 ? "1px solid rgba(255,255,255,0.05)" : "none" }}>
                        <div style={{ width: 22, height: 22, borderRadius: 6, background: DIM, border: `1px solid ${ACCENT}33`, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Space Mono', monospace", fontSize: 9, color: ACCENT, flexShrink: 0 }}>
