...
export default function LoginPage() {
  ...
  async function signInWithGoogle() {
    ...
    // on success, browser redirects to Google — no further code needed
  }

  if (checking) return null; // avoid flash before redirect
  ...
  return (
    <>
      ...
      <main style={{ minHeight:"100vh", background:"#050505", display:"flex", alignItems:"center", justifyContent:"center", position:"relative", overflow:"hidden" }}>
        ...
        {/* ambient glow */}
        <div style={{ position:"fixed", top:"30%", left:"50%", transform:"translate(-50%,-50%)", width:600, height:600, background:`radial-gradient(ellipse,rgba(200,255,0,0.04) 0%,transparent 70%)`, pointerEvents:"none" }} />

        {/* card */}
        <div className="fade-up" style={{ position:"relative", zIndex:1, width:"100%", maxWidth:400, margin:"0 24px" }}>

          {/* logo mark */}
          <div style={{ display:"flex", justifyContent:"center", marginBottom:40, animation:"float 4s ease-in-out infinite" }}>
            <div style={{ width:72, height:72, borderRadius:"50%", background:`linear-gradient(135deg,${ACCENT},#8FFF00)`, display:"flex", alignItems:"center", justifyContent:"center", boxShadow:`0 0 40px ${ACCENT}40, 0 0 80px ${ACCENT}20` }}>
              <span style={{ fontSize:32, color:"#050505" }}>⬡</span>
            </div>
          </div>

          {/* heading */}
          <div style={{ textAlign:"center", marginBottom:40 }}>
            <h1 style={{ fontFamily:"'Syne',sans-serif", fontWeight:800, fontSize:36, color:"white", letterSpacing:"-0.02em", lineHeight:1.1, marginBottom:12 }}>
          ...
          <div style={{ background:"rgba(255,255,255,0.03)", border:"1px solid rgba(255,255,255,0.08)", borderRadius:20, padding:32, backdropFilter:"blur(20px)" }}>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:10, marginBottom:28 }}>
              {[
                ...
                <div key={f.label} style={{ display:"flex", alignItems:"center", gap:10 }}>
                  <span style={{ fontSize:13, color:f.color, width:20, textAlign:"center" }}>{f.icon}</span>
            ...
            <button className="google-btn" onClick={signInWithGoogle} disabled={loading}>
              {loading ? (
                <>
                  <div style={{ width:18, height:18, border:"2px solid rgba(0,0,0,0.2)", borderTopColor:"#1a1a1a", borderRadius:"50%", animation:"spin 0.8s linear infinite" }} />
            ...
            {error && (
              <div style={{ marginTop:14, padding:"10px 14px", background:"rgba(255,68,68,0.08)", border:"1px solid rgba(255,68,68,0.2)", borderRadius:8, fontFamily:"'Space Mono',monospace", fontSize:10, color:"#FF6B6B" }}>
                ⚠ {error}
              </div>
            )}

            <p style={{ fontFamily:"'Space Mono',monospace", fontSize:9, color:"rgba(255,255,255,0.2)", textAlign:"center", marginTop:20, lineHeight:1.6 }}>
